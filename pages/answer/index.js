...
Page({
  ...
  submit(){
    ...
    var allSecond = minute*60+second;
      wx.u.getSettingByKey('time').then(res3 => {
      console.log(res3.result[0]['value']);
      var allMin = res3.result[0]['value'];
      var lastSecond = parseInt(allMin) * 60 - allSecond;

  
      console.log('lastSecond' + lastSecond);


      var params = { 'menu': menu, 'score': score, 'secondOver': lastSecond, 'result': result, 'questionMenu': questionMenu, 'saveStatus': 1 }
      wx.u.addHistory(params).then(res => {
        console.log(res);
        this.setData({
          loading: false,
